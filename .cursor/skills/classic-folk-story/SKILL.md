...
根据用户请求的故事或类型，**先查 [200-stories-index.md](references/stories/200-stories-index.md) 确定该则所属分类**，再读取对应参考文件获取出处、人物、梗概、主题等细节后讲述。200 则已全部归入下列九类文档，无遗漏：
...
| 寓言警世 | [references/stories/fables.md](references/stories/fables.md) |
| 英雄传奇 | [references/stories/heroes.md](references/stories/heroes.md) |
| 民间智者 | [references/stories/folk-wisdom.md](references/stories/folk-wisdom.md) |
...
| 历史演义 | [references/stories/history-legends.md](references/stories/history-legends.md) |
...
用户指定故事名 → **先查 [200-stories-index.md](references/stories/200-stories-index.md)** 确认该则是否在 200 则内及所属分类，再读取对应分类文档（mythology / four-legends / fables / heroes / folk-wisdom / fairy-mortal / liaozhai / history-legends / zhiguai）获取出处、人物、梗概、主题后讲述。

用户要求「列出民间故事」「推荐一些」「200 个故事」等 → 提供或引用 [200-stories-index.md](references/stories/200-stories-index.md) 中的八类 200 则清单，可按类展示或按需筛选。
...
- 提到"给孩子讲" → 优先推荐：民间智者、寓言警世类
- 提到"爱情" → 四大民间传说
- 提到"神话" → 上古神话类
- 提到"有意思的/好玩的" → 寓言警世、民间智者类
- 提到"英雄/励志" → 英雄传奇类
...
- **mcp_task 调用要点**：
  ...
  - **prompt**：在 prompt 中明确写出（以下为必含内容）：
    ...
    3. **评测参数**：小说类型选「民间故事/传说」或与故事所属分类最接近的类型（参考 novel-editor 的 genre-focus），篇幅类型按 6000–8000 字视为短篇/中篇，作品阶段选「初稿」；其余按 novel-editor 步骤 2 取默认。
...
- 修订后可用 MCP **count_document** 核验字数仍落在 6000–8000 字区间。
...
**用户问"随便讲一个"**：先查 [told-stories.md](references/stories/told-stories.md)，从 200 则索引的九类中各选一个**未在已讲列表中的**推荐，让用户挑；若某类已讲完则可从已讲中忽略该类。
